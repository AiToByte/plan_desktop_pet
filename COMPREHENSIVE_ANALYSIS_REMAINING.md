...
### BUG-2: ESP32 Pixel pause 后无法 resume (功能缺失)
...
**修复建议**:  
在 action handler 中增加 `pixel_resume` case，调用 `pixel_player.play()` 或 `pixel_player.resume()`。
...
### BUG-5: Haptic driver I2C 写入不检查返回值 (硬件通信)
...
```cpp
_writeRegister(REG_MODE, MODE_INTERNAL);       // 行76 - 无返回值检查
_writeRegister(REG_LIBRARY, 0x06);              // 行80
_writeRegister(REG_FEEDBACK, 0xA0);             // 行83
_writeRegister(REG_CONTROL1, 0x93);             // 行86
_writeRegister(REG_CONTROL2, 0xF5);             // 行89
```

**修复建议**:  
让 `_writeRegister` 返回 `bool` 或 `esp_err_t`，初始化时检查返回值，失败时记录错误日志并标记 `_initialized = false`。

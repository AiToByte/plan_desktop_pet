...
// [OPT-4] PSRAM静态JSON解析缓冲区（替代malloc/free，消除堆碎片化风险）
// 4KB足够覆盖所有JSON消息（pixel_data chunk_base64由PSRAM池直接处理）
__attribute__((section(".psram"))) static char g_jsonParseBuf[4096];
static constexpr size_t JSON_PARSE_BUF_SIZE = sizeof(g_jsonParseBuf);
...
// 注意：不能用=0初始化！否则编译器将其放入.rtc.data段（每次冷启动清零）
// 必须保持未初始化状态，编译器才会将其放入.rtc.noinit段（冷启动保留）
...
void setup() {
    ...
    // 冷启动检测：首次上电时RTC.noinit区内容随机，用魔数判断
    if (s_crashMagic != CRASH_MAGIC) {
        // 首次上电或RTC区被擦除：初始化计数器
        s_crashCount = 0;
        s_crashMagic = CRASH_MAGIC;
        LOG_I("Cold boot detected, crash counter initialized\n");
    }
    ...
    LOG_I("初始化显示...");
    display.begin();
    display.setupVSync();  // [OPT-2] 启用TE硬件V-Sync中断（必须在begin()之后）
...
void parseServerData(String json) {
    ...
    if (type == "status") {
        ...
        // [OPT-4] 使用PSRAM静态缓冲区替代malloc，消除堆碎片化
        int jsonLen = json.length();
        if (jsonLen + 1 > (int)JSON_PARSE_BUF_SIZE) {
            LOG_E("JSON too large (%d > %d), skipping\n", jsonLen, (int)JSON_PARSE_BUF_SIZE);
            return;
        }
        char* writeableBuf = g_jsonParseBuf;
        ...
        if (!b64Data || b64Len == 0) { LOG_E("Pixel: empty base64 data"); return; }
        ...
        if (!g_pxlBuffer) { LOG_E("Pixel: buffer alloc failed"); return; }

...
| 2 | USB 数据线 | Type-C（支持数据传输） | 1 | ¥10 | 纯充电线无法烧录 |
...
> 首次编译会自动下载 ESP32 工具链和库依赖，耗时 5~15 分钟属正常。
...
> 串口模式下 ESP32 和 PC 通过 USB 线直连，无法使用 WiFi 自动发现。
...
ESP32 首次启动时没有 WiFi 配置，会自动进入配网模式。项目提供 **五种配网方式**，按推荐程度排列。

### 4.1 方式一：Web AP 配网（推荐）
...
> UDP 自动发现要求 ESP32 和 PC 在**同一子网**。如果路由器隔离了广播，需手动配置 PC IP。

### 4.4 方式四：mDNS 服务发现

**适用场景**：路由器支持 mDNS 且 ESP32 已连接 WiFi，但 UDP 广播被隔离

1. PC 端启动后注册 mDNS 服务 `deskpet.local`
2. ESP32 连接 WiFi 后，通过 mDNS 查询 `deskpet.local` 解析 PC IP 地址
3. 解析成功后自动发起 TCP 连接

**前提条件**：
- 路由器未禁用 mDNS（大多数家用路由器默认支持）
- ESP32 和 PC 在同一局域网

**排查方法**：
```bash
# 在 PC 端验证 mDNS 服务是否注册成功
# Windows (需安装 Bonjour):
dns-sd -B _deskpet._tcp local

# macOS:
dns-sd -B _deskpet._tcp local

# Linux:
avahi-browse -rt _deskpet._tcp
```

### 4.5 方式五：USB 串口直连

**适用场景**：无 WiFi 环境、调试阶段、或 WiFi 不稳定时的保底方案

1. 用 Type-C USB 线连接 ESP32 和 PC
2. 修改 PC 端 `config.json`：
   ```json
   {
     "communication": {
       "mode": "serial",
   ...
   ```
3. 启动 PC 端程序，自动通过串口与 ESP32 通信

> 串口模式无需配网，但也无法使用 WiFi 相关功能（如 OTA 升级、mDNS 发现）。
...
│  │    └─ Device #3 (Pet-Bird)  │   │
│  └──────────────────────────────┘   │
│  UDP 广播: 255.255.255.255:19877    │
└─────────────────────────────────────┘
         │          │          │
    WiFi TCP    WiFi TCP    WiFi TCP
         │          │          │
    ┌────┴───┐ ┌────┴───┐ ┌────┴───┐
    │ ESP32  │ │ ESP32  │ │ ESP32  │
    │ #1     │ │ #2     │ │ #3     │
...
1. **每台 ESP32 独立配网**：各设备通过 AP/BLE 配网，均指向同一 PC 的 IP 地址和端口（19876）
2. **PC 端自动识别**：当前代码已支持多连接，通过 `comm_manager` 的连接 ID 区分不同设备
3. **差异化配置**：每台设备可配置不同角色/表情/名称

### 6.3 注意事项

- PC 端 TCP Server 无需修改，天然支持多客户端并发连接
- 每台 ESP32 的 NVS 中保存独立的配网信息
- UDP 广播发现对所有设备同时生效，无需额外配置
- 建议为每台设备分配固定 IP（路由器 DHCP 绑定），便于排查

### 6.4 多设备管理

- 通过 PC 端日志可查看每台设备的连接状态和 IP
- 设备断线重连时，PC 端自动更新连接映射
- 可为不同设备推送不同的像素动画和状态信息
...
|--------|------|
| `PSRAM:` | PSRAM 初始化结果 |
| `[WiFiManager]` | WiFi 连接状态 |
| `[CommManager]` | TCP 通信状态 |
| `[BLEConfig]` | BLE 配网状态 |
| `[DisplayManager]` | LCD 显示状态 |
| `[TouchHandler]` | 触摸事件 |
| `[HapticDriver]` | 马达驱动状态 |
| `[SoundManager]` | 音频状态 |
| `heap` | 堆内存使用情况 |
